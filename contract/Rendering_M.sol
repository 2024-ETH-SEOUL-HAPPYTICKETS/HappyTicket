...
library NFTRenderer_M_Good {
    using Strings for uint256;

    function render(RenderParams_M memory params) internal pure returns (string memory) {
    ...
    function generateSVG(RenderParams_M memory params) internal pure returns (string memory) {
        string memory svg = string(
            abi.encodePacked(
                ...
                '</style>',
                '<rect width="300" height="480" fill="hsl(120,40%,40%)" />',
                '<rect x="30" y="30" width="240" height="420" rx="15" ry="15" fill="hsl(120,90%,50%)" stroke="#000" />',
    ...
    function generateSVG(RenderParams_M memory params) internal pure returns (string memory) {
        string memory svg = string(
            abi.encodePacked(
                ...
                '.date { font: normal 14px sans-serif; }',
                '</style>',
                '<rect width="300" height="480" fill="hsl(360,40%,40%)" />',
                '<rect x="30" y="30" width="240" height="420" rx="15" ry="15" fill="hsl(360,90%,50%)" stroke="#000" />',
                '<text x="39" y="120" class="musician" fill="#fff">',
                params.musician,
                '</text>',
                '<text x="39" y="165" class="title" fill="#fff">',
